...
use ray_tracer_rust::ray_tracer::tuples::Tuple;
...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("magnitude", |b| {
        b.iter(|| {
            let vec = Tuple::new_vector(1.0, 2.0, 3.0);
    ...
    group.bench_function("dot", |b| {
        b.iter(|| {
            let vec_a = Tuple::new_vector(1.0, 2.0, 3.0);
            let vec_b = Tuple::new_vector(2.0, 3.0, 4.0);
            let _ = black_box(Tuple::dot(&vec_a, &vec_b));
        })
    });
    ...
    group.bench_function("cross", |b| {
        b.iter(|| {
            let vec_a = Tuple::new_vector(1.0, 2.0, 3.0);
            let vec_b = Tuple::new_vector(2.0, 3.0, 4.0);
            let _ = black_box(Tuple::cross(&vec_a, &vec_b));
        })
    });

    group.bench_function("reflect", |b| {
        b.iter(|| {
            let vec = Tuple::new_vector(1.0, -1.0, 0.0);
            let normal = Tuple::new_vector(0.0, 1.0, 0.0);
            let _ = black_box(Tuple::reflect(&vec, &normal));
        })
    });
    ...
    group.bench_function("eq", |b| {
        b.iter(|| {
            let vec_a = Tuple::new_vector(1.0, 2.0, 3.0);
            let vec_b = Tuple::new_vector(1.0, 2.0, 3.0);
            let _ = black_box(vec_a == vec_b);
        })
    });
